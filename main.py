...
# ============================================================
# BINANCE FUTURES FEATURE ENGINE
# ============================================================
...
def _price_seconds():
    return list(binance_second_prices)

def _price_n_seconds_ago(seconds):
    rows = _price_seconds()
    if not rows:
        return None
    now_sec = rows[-1][0]
...
def _large_delta(window_sec=30):
    cutoff = now_ms() - int(window_sec * 1000)
    buy = 0.0
    sell = 0.0
    for ts, price, quote, sign in reversed(binance_trades):
        if ts < cutoff:
            break
        if quote < BINANCE_LARGE_TRADE_USD:
            continue
        if sign > 0:
            buy += quote
        else:
...
def binance_feature_snapshot(outcome):
    rows = _price_seconds()
    prices = [float(p) for _, p in rows]
    last = prices[-1] if prices else None

    p3 = _price_n_seconds_ago(3)
    p10 = _price_n_seconds_ago(10)

    ret3 = ((last / p3) - 1.0) if last and p3 else 0.0
    ret10 = ((last / p10) - 1.0) if last and p10 else 0.0

    ema9 = _ema(prices[-60:], 9) if prices else None
    ema21 = _ema(prices[-90:], 21) if prices else None
    ema_bias = ((ema9 / ema21) - 1.0) if ema9 and ema21 else 0.0
    rsi14 = _rsi(prices, 14) if prices else None

    flow10 = _signed_flow(10)
    flow30 = _signed_flow(30)
    book = _book_imbalance()
    large = _large_delta(30)

    direction = 1.0 if str(outcome).lower() == "up" else -1.0

    # Confirmation tests intentionally modest for the first out-of-sample run.
    mom_pass = (
        direction * ret10 >= 0.00012
        or direction * ret3 >= 0.00006
    )
    flow_pass = direction * flow10 >= 0.06
    book_pass = direction * book >= 0.06

    # Weighted score: fast/slow momentum + flow + depth + larger trades + EMA.
    raw_score = 0.0
    raw_score += 1.0 if direction * ret3 > 0 else -1.0
    raw_score += 1.0 if direction * ret10 > 0 else -1.0
    raw_score += 1.0 if direction * flow10 > 0 else -1.0
    raw_score += 0.5 if direction * flow30 > 0 else -0.5
    raw_score += 1.0 if direction * book > 0 else -1.0
    raw_score += 0.5 if direction * large > 0 else (-0.5 if large != 0 else 0.0)
    raw_score += 0.5 if direction * ema_bias > 0 else -0.5

    combo_pass = sum([mom_pass, flow_pass, book_pass]) >= 2
    score_pass = raw_score >= 2.0
    ...
    return {
        ...
        "flow_30s": flow30,
        "book_imbalance": book,
        "large_delta_30s": large,
        "score": raw_score,
        "mom_pass": int(bool(mom_pass)),
        "flow_pass": int(bool(flow_pass)),
        "book_pass": int(bool(book_pass)),
        "combo_pass": int(bool(combo_pass)),
        "score_pass": int(bool(score_pass)),
    }

def shadow_mode_pass(mode, f):
    if mode == "B1_MOM":
        return bool(f["mom_pass"])
    if mode == "B2_FLOW":
        return bool(f["flow_pass"])
    if mode == "B3_BOOK":
        return bool(f["book_pass"])
    if mode == "B4_COMBO":
        return bool(f["combo_pass"])
    if mode == "B5_SCORE":
        return bool(f["score_pass"])
    return False

def store_binance_features(condition, variant, asset, outcome, signal_type, f, signal_ms):
    with db() as conn:
        conn.execute("""
            INSERT INTO binance_signal_features(
                signal_ms, condition_id, variant, asset, outcome, signal_type,
                ret_3s, ret_10s, ema9, ema21, ema_bias, rsi14,
                flow_10s, flow_30s, book_imbalance, large_delta_30s, score,
                mom_pass, flow_pass, book_pass, combo_pass, score_pass
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (
            signal_ms, condition, variant["name"], asset, outcome, signal_type,
            f["ret_3s"], f["ret_10s"], f["ema9"], f["ema21"],
            f["ema_bias"], f["rsi14"], f["flow_10s"], f["flow_30s"],
            f["book_imbalance"], f["large_delta_30s"], f["score"],
            f["mom_pass"], f["flow_pass"], f["book_pass"],
            f["combo_pass"], f["score_pass"],
        ))
        conn.commit()

def record_shadow_trade(
    condition, variant, mode, asset, outcome, signal_type,
    filled, avg, gross, fee, total, accepted, reason, trade_ms
):
    with db() as conn:
        conn.execute("""
            INSERT INTO binance_shadow_trades(
                trade_ms, condition_id, variant, mode, asset, outcome,
                signal_type, filled_shares, avg_price, gross_cost, fee,
                total_cost, accepted, reason
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (
            trade_ms, condition, variant["name"], mode, asset, outcome,
            signal_type, filled if accepted else 0.0,
            avg if accepted else None,
            gross if accepted else 0.0,
            fee if accepted else 0.0,
            total if accepted else 0.0,
            int(bool(accepted)), reason,
        ))
        conn.commit()

def process_binance_shadow_filters(
    condition, variant, asset, outcome, signal_type,
    filled, avg, gross, fee, total, signal_ms
):
    f = binance_feature_snapshot(outcome)
    store_binance_features(
        condition, variant, asset, outcome, signal_type, f, signal_ms
    )

    for mode in BINANCE_FILTER_MODES:
        key = (condition, variant["name"], mode)
        sides = shadow_accepted_sides[key]
        feature_pass = shadow_mode_pass(mode, f)
        ...
        record_shadow_trade(
            condition, variant, mode, asset, outcome, signal_type,
            filled, avg, gross, fee, total, accepted, reason, signal_ms
        )
...
async def binance_ws_loop():
    ...
    while True:
        try:
            async with websockets.connect(
                ...
                async for raw in ws:
                    ...
                    if "aggtrade" in stream.lower() or payload.get("e") == "aggTrade":
                        ts = si(payload.get("T") or payload.get("E") or now_ms())
            ...
            log.warning("BINANCE WS reconnect: %s", e)
            await asyncio.sleep(1)
...
def make_report(start_ts, end_ts):
    ...
    with db() as conn:
        ...
        binance_features_rows = conn.execute("""
            SELECT * FROM binance_signal_features
            WHERE signal_ms>=? AND signal_ms<?
            ORDER BY signal_ms
        """, (sm, em)).fetchall()
    ...
    with zipfile.ZipFile(path, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        z.writestr("binance_signal_features.csv", csv_bytes(binance_features_rows))
...
async def main():
    ...
    session = aiohttp.ClientSession(headers={
        "User-Agent": "PowerwinnerInspiredStrategySimulator/1.8-binance",
