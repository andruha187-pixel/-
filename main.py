...
async def poll_trades():
    ...
    while True:
        try:
            # Small overlap handles API delivery lag and out-of-order visibility.
            last_ts = safe_int(get_state("last_trade_ts", "0"))
            if last_ts <= 0:
                # On Render Free the local DB disappears after some restarts/deploys.
                # Re-fetch recent public trades so the current-hour report can recover.
            ...
            if isinstance(rows, list):
                rows = sorted(rows, key=lambda x: safe_int(x.get("timestamp")))
                new_count = 0
                max_ts = last_ts

                for t in rows:
                    if str(t.get("proxyWallet", "")).lower() not in ("", WALLET):
                        continue
                ...
                if new_count:
                    log.info("New trades stored: %d", new_count)
                elif first_run:
                    log.info("No new trades in initial window")
                first_run = False
        except Exception:
...
async def hourly_reporter():
    global last_report_end

    saved = safe_int(get_state("last_report_end", "0"))
    if saved > 0:
        last_report_end = saved
    else:
        # Render Free can lose local state on restart. Start from the beginning
        # of the current UTC hour; poll_trades backfills recent public trades.
    ...
    while True:
        boundary = next_hour_boundary()
        await asyncio.sleep(max(1, boundary - now_ts() + 3))
        ...
        try:
            ...
            with db_connect() as conn:
                count = conn.execute(
                    "SELECT COUNT(*) c FROM trades WHERE trade_ts>=? AND trade_ts<?",
                    (start_ts, end_ts),
                ).fetchone()["c"]
            caption = (
                f"Powerwinner hourly report\n"
                f"{utc_iso(start_ts)} → {utc_iso(end_ts)}\n"
                f"Trades: {count}"
            )
            await telegram_send_file(path, caption)
            last_report_end = end_ts
            set_state("last_report_end", end_ts)
        except Exception:
...
async def startup_message():
    ...
    await telegram_send_text(
        "🟢 Powerwinner Wallet Observer v2 FREE запущен.\n"
        f"Кошелёк: {WALLET}\n"
        "Отчёт: каждый час ZIP-файлом.\n"
        "Сделки собираются с takerOnly=false.\n"
        "MAKER/TAKER определяется эвристически по публичному стакану."
    )
