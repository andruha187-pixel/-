    ...
    while True:
        try:
            async with websockets.connect(BINANCE_WS,ping_interval=20,ping_timeout=20,close_timeout=5,max_size=10_000_000) as ws:
                log.info('BINANCE V4 WS connected | %s', BINANCE_SYMBOL.upper())
                async for raw in ws:
            ...
            log.warning('BINANCE V4 WS reconnect: %s', e)
            await asyncio.sleep(1)
...
async def health(request):
    ...
    return web.json_response({
        "ok": True,
        "version": "4.0-binance-fingerprint",
        "symbol": SYMBOL,
...
async def main():
    ...
    session = aiohttp.ClientSession(headers={
        "User-Agent": "PowerwinnerInspiredStrategySimulator/4.0-binance-fingerprint",
