...
assert bot.DECISION_INTERVAL == 1.0
assert bot.MOMENTUM_LOOKBACK_SEC == 6.0
...
# 1-second sampling must still measure momentum versus ~6 seconds ago,
# not versus only two 1-second ticks ago.
mtiming=make_market('BNB','timing')
ms=bot.now_ms()
h=bot.price_history[mtiming['condition_id']][mtiming['up_asset']]
h.clear()
h.extend([
    (ms-6000,.62),(ms-5000,.63),(ms-4000,.64),(ms-3000,.65),
    (ms-2000,.66),(ms-1000,.68),(ms,.69),
])
mom,ref=bot.momentum_for(mtiming['condition_id'],mtiming['up_asset'],2)
assert abs(ref-.62)<1e-12 and abs(mom-.07)<1e-12
# With less than the configured 6-second history, there must be no momentum yet.
h.clear(); h.extend([(ms-5000,.63),(ms-4000,.64),(ms-3000,.65),(ms-2000,.66),(ms-1000,.68),(ms,.69)])
mom,ref=bot.momentum_for(mtiming['condition_id'],mtiming['up_asset'],2)
assert mom is None and ref is None
