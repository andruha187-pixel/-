...
def _compute_store(slug, asset, start, b, k1t, k1, h_up, h_dn, res, traded):
    if traded:
        rows = db.query("SELECT key, ts FROM trades WHERE slug=? AND type='TRADE' "
                        "AND source IN ('backfill','live_nofeat')", (slug,))
        upd = [(_js(hist_feat(start, ts - DECISION_LAG_SEC, b, k1t, k1, h_up, h_dn, res)), key) for key, ts in rows]
        db.executemany("UPDATE trades SET feat=?, source='backfill' WHERE key=?", upd)
    snaps = []
