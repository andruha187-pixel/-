...
class PMBook:
    ...
    def __init__(self):
        ...
        self.last_msg = 0
        self.listeners = []  # cb(token, side, price, size, ts) на каждую сделку
    ...
    def _apply(self, tok, side, price, size):
        ...
        p, s = round(float(price), 3), float(size)
        if s <= 0:
            book.pop(p, None)
        else:
            book[p] = s
    ...
    def _handle(self, raw):
        ...
        for it in (o if isinstance(o, list) else [o]):
            ...
            if et == "book":
                ...
                self.books[tok] = {"b": {round(float(x["price"]), 3): float(x["size"]) for x in bids},
                                   "a": {round(float(x["price"]), 3): float(x["size"]) for x in asks}}
                ...
                try:
                    tr = (now, it.get("asset_id"), str(it.get("side", "")).upper(),
                ...
                for cb in self.listeners:
                    try:
                        cb(tr[1], tr[2], tr[3], tr[4], now)
                    except Exception as e:
                        log.warning("listener: %s", e)

    def level(self, tok, price, side="b"):
        b = self.books.get(tok)
        return b[side].get(round(price, 3), 0.0) if b else 0.0
